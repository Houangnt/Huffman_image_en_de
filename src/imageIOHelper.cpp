...
Image* ImageIOHelper::readImage(const string& filename) {
    ...
    // Check if the image is true color or grayscale
    bool isTrueColor = (bitSize == BitmapTrueColorBitSize);

    // Calculate the expected data size based on the image type
    uint32_t dataSize;
    if (isTrueColor) {
        dataSize = 3 * width * height;  // 3 Bytes per pixel for true color image
    } else {
        dataSize = width * height;      // 1 Byte per pixel for grayscale image
    }
    ...
    fread(rawDataPtr, sizeof(unsigned char), rawSize, fp);
    fclose(fp);

    // Process the image data based on its type
    if (isTrueColor) {
        ...
        // Simply copy the raw data to the output data pointer
        memcpy(dataPtr, rawDataPtr, dataSize);
    }
